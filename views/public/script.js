const colorMas = ["#ef6e69","#f077a2","#8f6dcb","#5eb3f6","#67d7e5","#ffe083"];
...
function addToHTML(title, value, color) {
  document.getElementById("luToDO").insertAdjacentHTML('beforeend',`
    <li id = '${title}' class = 'todo-list' style = 'background-color : ${color}'=>
      <div class = 'view'>
...
function editColor(elem) {
  const luChild = document.getElementById("luToDO").querySelectorAll(".todo-list li");
  let masCheck = [];
  for (let i = 0; i < luChild.length; i++){
    if (document.getElementById(luChild[i].id).querySelector(".checkbox input").checked){
      ...
      masCheck.push({ id , color });
      document.getElementById(id).querySelector(".checkbox input").checked = false;
    }
  };
